...
const Head = () => {
  ...
  return (
    <div className="sticky top-0 grid grid-flow-col py-3 md:px-3 px-1 bg-white dark:bg-slate-800 w-full shadow-lg">
      ...
      <div className={`flex col-span-2 md:col-span-1 items-center mx-4 ${smSearch?'max-sm:hidden':''}`}>
        ...
        className="text-xl md:text-[1.75rem] dark:text-white cursor-pointer"/>
        <Link to="/">
        <img className="h-4 md:h-[1.3rem] md:mx-6 mx-1" alt="youtubeLogo" src={darkMode? "assets/yt-logoDrakMode.png": "assets/youtubelogoLightMode.png"}>
        </img></Link>
      </div>

      {/* Search */}
      <div className={`${smSearch?'col-span-10':'col-span-10 max-sm:flex max-sm:justify-end'} ml-1 md:ml-32`}>
      ...
      {/* DarkMode Icon */}
      <div className={`flex items-center col-span-2 justify-end cursor-pointer ${smSearch?'max-sm:hidden':'ml-2'}`}>
      ...
      {/* User Icon */}
      <div className={`flex justify-center md:col-span-1 col-span-2 cursor-not-allowed items-center ${smSearch?'max-sm:hidden':''}`}>
        <BiUserCircle className='text-3xl md:text-[2rem] font-normal dark:text-white'/>
      </div>
    </div>
  )
}
