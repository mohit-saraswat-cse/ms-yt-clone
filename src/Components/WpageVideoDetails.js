...
import { videoDetailsApi } from '../config/constants';
...
const WpageVideoDetails = (props) => {
    ...
    const fetchData=async ()=>{
        const data=await fetch(videoDetailsApi+"&id="+props.videoId);
        const json=await data.json();
        setvideoDetails(json?.items[0]);
        props.channelDetail(json?.items[0]?.snippet?.channelId);
    }
        ...
        <div className='flex flex-col w-full'>
            ...
            <div className='bg-gray-100 p-2 rounded-xl px-4 dark:bg-slate-600 '>
                ...
                <div className='lg:flex'>
                    ...
                    <p className='lg:ml-6 font-medium'>{PublishedTimeOfVideo(publishedAt)}</p>
            ...
            {/* comment Section */}
            <div className='p-2'>
                <VideoCommentContainer videoId={props.videoId} commentCount={commentCount}/>
            </div>
        </div>
